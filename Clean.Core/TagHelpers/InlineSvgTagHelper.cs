...
{
    /// <summary>
    /// This was borrowed from the package Our.Umbraco.TagHelpers and isolated solely for this starter kit.
    ...
    public class InlineSvgTagHelper : TagHelper
    {
        ...
        public InlineSvgTagHelper(MediaFileManager mediaFileManager, IWebHostEnvironment webHostEnvironment, IPublishedUrlProvider urlProvider, AppCaches appCaches)
        {
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            if (Cache)
            {
                ...
                var cacheMins = CacheMinutes > 0 ? CacheMinutes : 0;
        ...
        private string? GetFileContents()
        {
            ...
            if (EnsureViewBox || !string.IsNullOrEmpty(CssClass))
            {
                ...
                foreach (var svgNode in svgs)
                {
                    ...
                    if (EnsureViewBox && svgNode.Attributes.Contains("width") && svgNode.Attributes.Contains("height") && !svgNode.Attributes.Contains("viewbox"))
                    {
                        var width = GetDecimal(svgNode.GetAttributeValue("width", "0"));
                        var height = GetDecimal(svgNode.GetAttributeValue("height", "0"));
